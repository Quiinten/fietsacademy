...
public class DocentRepository extends AbstractRepository {
	...
	public List<Docent> findByWeddeBetween(BigDecimal van, BigDecimal tot, int vanafRij, int aantalRijen) {
		return getEntityManager().createNamedQuery("Docent.findByWeddeBetween", Docent.class).setParameter("van", van)
				...
				.getResultList();
		/*
		 * Als je een query hebt met het in sleutelwoord en daarin een parameter select
		 * d from Docent d where d.voornaam in (:voornamen) vul je deze parameter met
		 * een List of Set: .setParameter("voornamen", Arrays.asList("Jules",
		 * "Cleopatra")); Deze query geeft de docenten terug met Jules of Cleopatra als
		 * voornaam.
		 */
	}
